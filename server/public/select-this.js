...
Object.defineProperty(exports, "__esModule", { value: true });
var generate_1 = __webpack_require__(2);
...
Object.defineProperty(exports, "__esModule", { value: true });
var accessors_1 = __webpack_require__(3);
...
function generateSelector(selector) {
    ...
    if (selector.selector) {
        ...
        var selectors = [generateDefaultSelector(element, selector, false)];
        if (element.parents) {
            for (var i = 0; i < element.parents.length - 1; i++) {
                var parentElement = element.parents[i];
...
function generateDefaultSelector(element, selector, forceSkipClass) {
    var selectorBuilder = element.tag;
...
function generateElementInfo(element) {
    ...
    var elementInfo = getElementModel(element);
    // Get parent element information
    var parents = [];
    var currentElement = element.parentElement;
    while (helpers_1.isElement(currentElement) && accessors_1.getTagSelector(currentElement) !== 'body') {
...
exports.generateElementInfo = generateElementInfo;
function getElementModel(element) {
    var elementInfo = {};
    // get tag name
    elementInfo.tag = accessors_1.getTagSelector(element);
    // get id
    var id = accessors_1.getIdSelector(element);
    if (!helpers_1.isNull(id)) {
        elementInfo.id = id;
        if (helpers_1.isUnique(element, id)) {
            elementInfo.uid = true;
        }
    }
    // get class
    var classes = accessors_1.getClassName(element);
    if (classes && classes.length) {
        elementInfo.classes = classes;
    }
    // get text
    var text = accessors_1.getTextSelector(element);
    if (!helpers_1.isNull(text)) {
        elementInfo.text = text;
    }
    // get position
    var position = Number(accessors_1.getNthChildSelector(element, false));
    if (position > 1) {
 ...
 * @return { Boolean }
 */
function isUniqueSibling(element, selector) {
    var parentNode = element.parentElement;
    var elements = parentNode.querySelectorAll(selector);
...
exports.isUniqueSibling = isUniqueSibling;
...
function getClassSelectors(element) {
    ...
    return classes.map(function (className) { return "." + className; });
}
 ...
 * @return { Array }
 */
function getNthChildSelector(element, asSelector) {
    if (asSelector === void 0) { asSelector = true; }
    var counter = 0;
    var k;
    var sibling;
    var parentNode = element.parentNode;
    if (!helpers_1.isNull(parentNode)) {
        var childNodes = parentNode.childNodes;
        var len = childNodes.length;
        for (k = 0; k < len; k++) {
            sibling = childNodes[k];
            if (helpers_1.isElement(sibling)) {
                counter++;
                if (sibling === element) {
